...
class Jugador():
    def __init__(self, coordenada_x, coordenada_y, frame_rate = 100, speed_walk = 6, speed_run = 12,gravity = 28, delta_ms = 100, speed_jump = 20)-> None:
        ...
        """COLISIONS"""############
        ########################
        self.rect_colision = pg.Rect(coordenada_x, coordenada_y, 40, 80)
    ...
    def moviment_control(self, key_get_pressed, delta_ms):
        #GET PRESSED KEY
        movement_in_x = 0
        movement_in_y = 0
        ...
        self.rect_colision.x = self.__rect.x+45
        self.rect_colision.y = self.__rect.y+50
        # self.__set_edges_limits()
        # self.__set_edge_limits_x()
        # self.__set_edge_limits_y()
    def draw(self, screen):
        self.__actual_image_animation = self.__actual_animation[self.__initial_frame] #animacion actual
        self.__rect.y = min(self.__rect.y, ALTO_VENTANA - self.__actual_image_animation.get_height()) #posicion actual de la imagen
        # pg.draw.rect(screen, (255,00,00), self.__rect)
        pg.draw.rect(screen, (255, 255, 255), self.rect_colision)
        screen.blit(self.__actual_image_animation, self.__rect) #bliteamos imagen actual

    def update(self,key_get_pressed, delta_ms,screen):
        #DRAW PLAYER
        self.draw(screen)
        
        
        #UPDATE PLAYER COORDINATES
        self.moviment_control(key_get_pressed, delta_ms)
        #ANIMATION
